...
This function specifically plots a *numeric* variable, `y`, by a *categorical* variable, `x`, so we will add an error trap with the input variables are not of the correct class. As for missing values, we will specify that NA values should be removed -- but no siliently! We want missing values to be removed *with* a warning that this has occured -- telling us exactly how many rows were removed in making the plot so that we can check that makes sense. This will be specified in the `fct_reorder` part of the function as .na_rm = NULL (this removes NAs but generates a warning). We are doing this because we are just plotting one variable by a categorical one, so rows that are missing either piece of information should always be removed. 
